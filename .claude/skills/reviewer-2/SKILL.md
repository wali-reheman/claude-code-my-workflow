...
1. **Detects** your research design type from project files (R code, manuscripts, slides)
...
- Check slide pedagogy (that's `pedagogy-reviewer` / `/devils-advocate`)
...
2. **Slides:** Glob `Slides/**/*.tex`, `Quarto/**/*.qmd`
...
- If no R code exists (qualitative project), detection relies on manuscript/slide text
...
| **`/create-lecture` (Phase 5)** | Add methodology review for research-methods lectures |
| **`/slide-excellence` (Agent 6)** | Can fill the substance review slot for methodology lectures |
