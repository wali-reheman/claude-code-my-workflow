...
**Cross-reference claims with code:** For every methodological claim in the manuscript/slides (e.g., "we use clustered standard errors"), verify it appears in the R code. For every estimator in the R code, verify it matches what the manuscript describes.
...
3. **Be fair.** Pedagogical simplifications in slides are fine. Smuggled causal claims are not.
