...
This protocol applies whenever working with papers that have replication packages (Stata, R, or other code). The goal is exact numerical reproducibility before any project-specific extensions.

---

## Why This Matters

- Incorrect replication = publishing wrong numbers
- Translation errors (Stata to R) can silently change results
- Extensions built on a broken baseline propagate errors
- Reviewers and readers will attempt to replicate -- they must get the same numbers
...
4. **Store gold standard** in one of:
   - `quality_reports/paper_name_replication_targets.md` (for reference)
...
4. **Save all intermediate results as RDS:**
   - Raw data after cleaning: `paper_name_data_clean.rds`
   - Estimation results: `paper_name_estimates.rds`
   - Summary tables: `paper_name_summary.rds`
...
Save to `quality_reports/paper_name_replication_report.md`:
...
- **`/create-paper` analysis phase:** When replication packages are provided, follow this protocol before extending
