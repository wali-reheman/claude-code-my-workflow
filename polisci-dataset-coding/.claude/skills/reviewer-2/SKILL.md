...
1. **Detects** your research design type from project files (R code, manuscripts, data documentation)
...
1. **Manuscripts:** Glob `Manuscripts/**/*.tex`, `Manuscripts/**/*.qmd`
2. **R scripts:** Glob `scripts/**/*.R`, `Replication/**/*.R`
3. **Data:** Glob `Replication/data/**/*`, `data/**/*`
4. **Coded data:** Glob `Replication/data/coded/**/*`
...
If the project has very few files (e.g., no R code or codebook), note this as a limitation — the review will be less thorough without code to cross-reference.

### Step 2: Design Detection (Phase 0)

Scan R scripts and manuscript/slide text for design signals. Use the detection heuristics from the `methodology-reviewer` agent specification.
...
- If no R code exists (qualitative project), detection relies on manuscript text and data documentation
...
|---|---|
| **Orchestrator (Step 3: REVIEW)** | Can be called during review step for manuscripts and datasets |
| **`r-reviewer` agent** | Complementary: r-reviewer checks code quality, `/reviewer-2` checks estimator choice |
| **`coding-reliability-reviewer` agent** | Complementary: reliability reviewer checks measurement, `/reviewer-2` checks design |
