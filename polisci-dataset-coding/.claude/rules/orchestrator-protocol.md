...
3. **Skill delegation** — when a skill like `/create-dataset` or `/prep-data` reaches its implementation phase, the orchestrator loop governs execution.

The orchestrator does NOT activate for:

- Single-file trivial edits (typo fix, add a citation)
- Purely informational questions
- Running a standalone skill like `/review-r` or `/commit`
...
| AI-coded dataset files (`Replication/data/coded/`) | coding-reliability-reviewer, r-reviewer | Yes |
...
- [e.g., "Run /review-r for R code review"]
```
