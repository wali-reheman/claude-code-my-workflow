...
| Persistent memory | CLAUDE.md + PROJECT_MEMORY.md survive across sessions |
...
The workflow stores corrections and learned facts in `PROJECT_MEMORY.md` at the repo root. This file is named `PROJECT_MEMORY.md` (not `MEMORY.md`) to avoid collision with Claude Code's built-in memory system at `~/.claude/projects/`. By keeping it in the repo, you can see, edit, and version-control all corrections.
...
Rules instruct Claude to read this file at session start and append to it with the Edit tool (not Claude's built-in `/memory` command).

### Plans --- Compression-Resistant Task Memory

While PROJECT_MEMORY.md stores long-lived project facts, **plans** store task-specific strategy. Every non-trivial plan is saved to `quality_reports/plans/` with a timestamp. This means:
...
With CLAUDE.md, PROJECT_MEMORY.md, plans, and session logs, the system has four distinct memory layers. Here is what each one does and when it matters:
...
| Corrections | `PROJECT_MEMORY.md` | Yes (on disk) | On `[LEARN]` tag | Prevent repeating past mistakes |
...
These tags are searchable and persist in `PROJECT_MEMORY.md` across sessions. When Claude encounters a similar situation, it checks memory first.
